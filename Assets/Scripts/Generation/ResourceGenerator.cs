...
public class ResourceGenerator : MonoBehaviour
{
    ...
    {
        float halfWorldSize = _worldSize / 2;
        float resourceThreshold = threshold + 0.075f;
        ...
        for (int y = 0; y < _worldSize; y++)
        {
            for (int x = 0; x < _worldSize; x++)
            {
                ...
                GameObject resource = _map[x, y] > resourceThreshold ? rockPrefab : treePrefab;

                Instantiate(resource, new Vector3(x - halfWorldSize, 0, y - halfWorldSize), Quaternion.identity/*Quaternion.Euler(0, Random.Range(0, 360), 0)*/, resourcesParent);
            }
        }
    }
    ...
    private void GenerateMap()
    {
        ...
        for (int y = 0; y < _worldSize; y++)
        {
            ...
            {
                if (!IsInsideBase(x, y))
                    _map[x, y] = Mathf.PerlinNoise(x / 2.88f, y / 2.88f) * spawnRate;
            }
        }
    }
    ...
    {
        return _bases.Any(v => Vector2.Distance(new Vector2(x, y), v) <= _baseRange);
